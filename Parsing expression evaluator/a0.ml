...
(*open Signature_a0
  ...
  (*Your code goes here*)
    ...
    let rec test (l:int list):int list = match l with
        []->[]
        |(x::xs)->if x=0 then test(xs)
                else l ;;       
    ...
    (*only called when l1>=l2 and give reversed list.... it may happen that it gives [8;0] in place of 8 so take care when u reverse it to remove head , For this I have made test function which is used wherever necessary*)
    ...
    let sub (l1:bigint) (l2:bigint) : bigint = (match l1 with
        (_,[])->(match l2 with
            (_,[]) -> (NonNeg,[])
            |(Neg,xs)->(NonNeg,test(xs))
            |(NonNeg,xs)->(Neg,test(xs)))
        |(Neg,xs)->(match l2 with
            (_,[])->l1
            |(NonNeg,ys) -> (Neg,test(rev (add_list xs ys 0)))
            |(Neg,ys) -> (if gt (NonNeg,xs) (NonNeg,ys) = true then (Neg,test(rev (sub_list xs ys 0)))
                                else if gt (NonNeg,ys) (NonNeg,xs) = true then (NonNeg,test(rev (sub_list ys xs 0)))
                                else (NonNeg,[]))) (*-a+a = []*)
        |(NonNeg,xs)->(match l2 with
                        (_,[])->(match l1 with
					(a,xs)->(a,test(xs)))
                        |(Neg,ys) -> (NonNeg,test(rev (add_list xs ys 0)))
                        |(NonNeg,ys) -> (if gt (NonNeg,xs) (NonNeg,ys) = true then (NonNeg,test(rev (sub_list xs ys 0)))
                                            else if gt (NonNeg,ys) (NonNeg,xs) = true then (Neg,test(rev (sub_list ys xs 0)))
                                            else (NonNeg,[]) ) )) (*-a+a = []*)
    ;; 
    ...
    let rec div_sub (l1:int list) (l2:int list) (c:int): int list * int list = match l1 with
        ...
        |_-> if (gt (NonNeg,l2) (NonNeg,l1)) = true then ([c],l1)
                ...
                else (div_sub (test(rev (sub_list l1 l2 0) )) l2 (c+1)) 
    ;; 
    ...
    let rec div_list_q (l1:int list) (l2:int list) (n:int list): int list = (match l1 with
        ...
        |_-> (if (gt (NonNeg,l2) (NonNeg,l1)) = true then n
                ...
                (if (gt (NonNeg,l2) (NonNeg,(take (leng l2) l1)) ) = false then
                    ...
                    div_list_q (test(rev (sub_list l1 (append (rev (List.tl(rev (mult_l l2 m)))) (leng l1 - leng l2)) 0))) l2 (rev (add_list (append m (leng l1 - leng l2)) n 0))
                else 
                let (m,k) = (div_sub (take ((leng l2)+1) l1) l2 0) in
                div_list_q (test(rev (sub_list l1 (append (rev (List.tl(rev (mult_l l2 m)))) (leng l1 - leng l2 -1)) 0))) l2 (rev (add_list (append m ((leng l1)-(leng l2) -1)) n 0)))
            ));;
